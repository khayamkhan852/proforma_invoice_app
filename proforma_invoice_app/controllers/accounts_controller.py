...
from proforma_invoice_app.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from proforma_invoice_app.controllers.sales_and_purchase_return import validate_return
from erpnext.exceptions import InvalidCurrency
from erpnext.setup.utils import get_exchange_rate
from erpnext.stock.doctype.item.item import get_uom_conv_factor
from erpnext.stock.doctype.packed_item.packed_item import make_packing_list

from proforma_invoice_app.general_functions.stock.get_item_details import (
...
class AccountsController(TransactionBase):
	...
	def validate_all_documents_schedule(self):
		...
		elif self.doctype in ("Quotation", "Purchase Order", "Sales Order", "Proforma"):
	...
	def calculate_taxes_and_totals(self):
		...
		from proforma_invoice_app.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def set_missing_item_details(self, for_validate=False):
		...
		if hasattr(self, "items"):
			...
			if self.doctype in ["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "Proforma"]:
	...
	def on_cancel(self):
		...
		elif self.doctype in ["Sales Order", "Purchase Order", "Proforma"]:
			if frappe.db.get_single_value(
				"Accounts Settings", "unlink_advance_payment_on_cancelation_of_order"
			):
				unlink_ref_doc_from_payment_entries(self)

			if self.doctype == "Sales Order" or self.doctype == "Proforma":
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from proforma_invoice_app.controllers.status_updater import get_allowance_for
	...
	def get_party(self):
		party_type = None
		if self.doctype in ("Opportunity", "Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "Proforma"):
			party_type = "Customer"
	...
	def validate_payment_schedule_dates(self):
		...
		for d in self.get("payment_schedule"):
			if (self.doctype in ["Sales Order", "Proforma"]) and getdate(d.due_date) < getdate(self.transaction_date):
				frappe.throw(
	...
	def is_internal_transfer(self):
		...
		"""
		if self.doctype in ("Sales Invoice", "Delivery Note", "Sales Order", "Proforma"):
...
def set_order_defaults(parent_doctype, parent_doctype_name, child_doctype, child_docname, trans_item):
	...
	date_fieldname = "delivery_date" if child_doctype in ["Sales Order Item", "Proforma Item"] else "schedule_date"
	...
	if child_doctype == "Sales Order Item" or child_doctype == "Proforma Item":
		child_item.warehouse = get_item_warehouse(item, p_doc, overwrite_warehouse=True)
...
def validate_child_on_delete(row, parent):
	...
	if parent.doctype == "Sales Order" or parent.doctype == "Proforma":
...
def update_bin_on_delete(row, doctype):
	...
	if doctype == "Sales Order" or doctype == "Proforma":
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	def get_new_child_item(item_row):
		child_doctype = (
    		"Sales Order Item" if parent_doctype == "Sales Order" 
    		else "Proforma Item" if parent_doctype == "Proforma" 
	...
	def validate_quantity(child_item, new_data):
		...
		if parent_doctype in ["Sales Order", "Proforma"] and flt(new_data.get("qty")) < flt(child_item.delivered_qty):
	...
	for d in data:
		...
		if not d.get("docname"):
			...
			if parent_doctype == "Sales Order" or parent_doctype == "Proforma":
		...
		if d.get("delivery_date") and parent_doctype in ["Sales Order", "Proforma"]:

...
from proforma_invoice_app.controllers.accounts_controller import get_taxes_and_charges
from proforma_invoice_app.controllers.sales_and_purchase_return import get_rate_for_return
from proforma_invoice_app.controllers.stock_controller import StockController

from erpnext.stock.doctype.item.item import set_item_default
from proforma_invoice_app.general_functions.stock.get_item_details import get_bin_details, get_conversion_factor
...
class SellingController(StockController):
	...
	def onload(self):
		super().onload()
		if self.doctype in ("Sales Order", "Delivery Note", "Sales Invoice", "Proforma"):
	...
	def get_so_qty_and_warehouse(self, so_detail):
		so_item = frappe.db.sql(
			"""select qty, warehouse from `tabProforma Item`
	...
	def check_sales_order_on_hold_or_close(self, ref_fieldname):
		for d in self.get("items"):
			if d.get(ref_fieldname):
				status = frappe.db.get_value("Proforma", d.get(ref_fieldname), "status")
				if status in ("Closed", "On Hold") and not self.is_return:
					frappe.throw(_("Proforma {0} is {1}").format(d.get(ref_fieldname), status))
	...
	def update_reserved_qty(self):
		...
		for so, so_item_rows in so_map.items():
			if so and so_item_rows:
				sales_order = frappe.get_doc("Proforma", so)

				if (sales_order.status == "Closed" and not self.is_return) or sales_order.status in [
					"Cancelled"
				]:
					frappe.throw(
						_("{0} {1} is cancelled or closed").format(_("Proforma"), so),
	...
	def set_gross_profit(self):
		if self.doctype in ["Sales Order", "Quotation", "Proforma"]:
			for item in self.items:
	...
	def validate_for_duplicate_items(self):
		...
		for d in self.get("items"):
			...
			elif self.doctype in ["Sales Order", "Quotation", "Proforma"]:

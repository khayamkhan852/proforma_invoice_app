...
erpnext.accounts.taxes.setup_tax_validations("Proforma");
erpnext.sales_common.setup_selling_controller();

frappe.ui.form.on("Proforma", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0) {
				frappe.call({
                    ...
                    method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
		...
		// Hide `Reserve Stock` field description in submitted or cancelled Proforma.
		if (frm.doc.docstatus > 0) {
	...
	onload: function (frm) {
		...
		// On cancel and amending a Proforma with advance payment, reset advance paid amount
		if (frm.is_new()) {
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					onchange: () => {
						let proforma_item = dialog.get_value("add_item");

						if (proforma_item) {
							frm.doc.items.forEach((item) => {
								if (item.name === proforma_item) {
									let unreserved_qty =
										(flt(item.stock_qty) -
									...
									if (unreserved_qty > 0) {
										dialog.fields_dict.items.df.data.forEach((row) => {
											if (row.proforma_item === proforma_item) {
				...
				{ fieldtype: "Section Break" },
				{
					...
					fields: [
						{
							fieldname: "proforma_item",
							fieldtype: "Link",
							label: __("Proforma Item"),
							options: "Proforma Item",
							reqd: 1,
							in_list_view: 1,
							get_query: () => {
							...
							onchange: (event) => {
								if (event) {
									...
									frm.doc.items.forEach((item) => {
										if (item.name === item_row.proforma_item) {
...
frappe.ui.form.on("Proforma Item", {
...
erpnext.selling.SalesOrderController = class SalesOrderController extends erpnext.selling.SellingController {
			...
			this.frm.add_custom_button(
				...
				function () {
					let d = erpnext.utils.map_current_doc({
						method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_proforma",
					...
					setTimeout(() => {
						d.$parent.append(`
							<span class='small text-muted'>
								${__("Note: Please create Proformas from individual Quotations to select from among Alternative Items.")}
							</span>
					`);
					}, 200);
				},
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.get_work_order_items",
			args: {
				proforma: this.frm.docname,
			},
			freeze: true,
			callback: function (r) {
				if (!r.message) {
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_material_request",
			frm: this.frm,
		});
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
                    method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						proforma: me.frm.docname,
	...
	make_delivery_note(delivery_dates, for_reserved_stock = false) {
		frappe.model.open_mapped_doc({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_sales_invoice() {
		frappe.model.open_mapped_doc({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_sales_invoice",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_inter_company_purchase_order",
			frm: this.frm,
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.make_maintenance_visit",
			frm: this.frm,
		});
	}
	...
	make_purchase_order() {
		...
		if (!pending_items) {
			frappe.throw({
				message: __("Purchase Order already created for all Proforma items"),
				title: __("Note"),
			});
		}
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action(args) {
				...
				var method = args.against_default_supplier
					...
					: "make_purchase_order";
				return frappe.call({
                    method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma." + method,
	...
	update_status(label, status) {
		...
		frappe.call({
            method: "proforma_invoice_app.proforma_invoice_app.doctype.proforma.proforma.update_status",

...
class ProformaInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		from proforma_invoice_app.proforma_invoice_app.doctype.proforma_invoice_item.proforma_invoice_item import ProformaInvoiceItem
...
@frappe.whitelist()
def make_proforma_invoice(source_name: str, target_doc=None):
...
def _make_proforma_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	def can_map_row(item) -> bool:
		"""
		Row mapping from Quotation to Proforma Invoice:
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Quotation": {"doctype": "Proforma Invoice", "validation": {"docstatus": ["=", 1]}},
			"Quotation Item": {
...
@frappe.whitelist()
def close_or_unclose_proforma_invoice(names, status):
...
def make_material_request(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			...
			"Proforma Invoice Item": {
				"doctype": "Material Request Item",
				"field_map": {"name": "proforma_invoice_item", "parent": "proforma_invoice"},
...
def create_stock_reservation_entries_for_so_items(
	...
	if not from_voucher_type and (
		proforma_invoice.get("_action") == "submit"
		and proforma_invoice.set_warehouse
		and cint(frappe.get_cached_value("Warehouse", proforma_invoice.set_warehouse, "is_group"))
	):
		return frappe.msgprint(
			_("Stock cannot be reserved in the group warehouse {0}.").format(
				frappe.bold(proforma_invoice.set_warehouse)
			)
		)

	validate_stock_reservation_settings(proforma_invoice)
	...
	if items_details:
		for item in items_details:
			so_item = frappe.get_doc("Proforma Invoice Item", item.get("proforma_invoice_item"))
	...
	sre_count = 0
	reserved_qty_details = get_sre_reserved_qty_details_for_voucher("Proforma Invoice", proforma_invoice.name)

	for item in items if items_details else proforma_invoice.get("items"):
		...
		sre.voucher_type = proforma_invoice.doctype
		sre.voucher_no = proforma_invoice.name
		sre.voucher_detail_no = item.name
		sre.available_qty = available_qty_to_reserve
		sre.voucher_qty = item.stock_qty
		sre.reserved_qty = qty_to_be_reserved
		sre.company = proforma_invoice.company
		sre.stock_uom = item.stock_uom
		sre.project = proforma_invoice.project
...
def validate_stock_reservation_settings(voucher: object) -> None:
	...
	allowed_voucher_types = ["Proforma Invoice"]
